...
from api.third_party.binance.error import ClientError, ServerError
from api.third_party.binance.lib.utils import get_timestamp
from api.third_party.binance.lib.utils import cleanNoneValue
from api.third_party.binance.lib.utils import encoded_string
from api.third_party.binance.lib.utils import check_required_parameter
from api.third_party.binance.lib.authentication import hmac_hashing, rsa_signature
...
class API(object):
    ...
    async def queryAsync(self, url_path, payload=None):
        return await self.send_request_async("GET", url_path, payload=payload)
    ...
    def send_request(self, http_method, url_path, payload=None, special=False):
        ...
        self._handle_exception(response, url + '\n' + str(payload))
    ...
    async def send_request_async(self, http_method, url_path, payload=None, special=False):
        if payload is None:
            payload = {}
        url = self.base_url + url_path
        logging.debug("url: " + url)
        ...
        async with ClientSession() as session:
            ...
            async with self._dispatch_request_async(session, http_method)(**params) as response:
                text = await response.text()
                logging.debug("raw response from server:" + text)
                await self._handle_exception_async(response, url + '\n' + str(payload), text)
    ...
    def _dispatch_request_async(self, session, http_method):
        return {
            "GET": session.get,
    ...
    def _handle_exception(self, response, url):
        ...
        if 400 <= status_code < 500:
            try:
                ...
                raise ClientError(status_code, None, url + '\n' + response.text, response.headers)
            raise ClientError(status_code, err["code"], url + '\n' + err["msg"], response.headers)
        raise ServerError(status_code, url + '\n' + response.text)


    async def _handle_exception_async(self, response, url, text):
